...
import { ethChainId, foxAssetId, jinxAssetId } from '@shapeshiftoss/caip'
...
export const FuryPage = () => {
  ...
  const assetFury = useAppSelector(state => selectAssetById(state, foxAssetId))
  const assetJinx = useAppSelector(state => selectAssetById(state, jinxAssetId))
  if (!assetFury) throw new Error(`Asset not found for AssetId ${foxAssetId}`)
  ...
  const furyFilter = useMemo(() => ({ assetId: foxAssetId }), [])
