...
import type { ExternalOpportunity } from '../../FuryCommon'

type FuryOtherOpportunityPanelRowProps = {
  opportunity: ExternalOpportunity
}

export const FuryOtherOpportunityPanelRow: React.FC<FuryOtherOpportunityPanelRowProps> = ({
  opportunity,
}) => {
  const {
  ...
  const opportunityButtonTranslation = useMemo(() => {
    if (opportunity.link) return 'plugins.furyPage.getStarted'
    if (isDemoWallet || !wallet || !supportsETH(wallet)) return 'common.connectWallet'

    return hasActivePosition ? 'plugins.furyPage.manage' : 'plugins.furyPage.getStarted'
