...
export const Confirm: React.FC<ConfirmProps> = ({ accountId, onNext }) => {
  ...
  const { unstake } = useFuryFarming(contractAddress)
  ...
  const handleConfirm = useCallback(async () => {
    try {
      if (
        ...
        return
      dispatch({ type: FuryFarmingWithdrawActionType.SET_LOADING, payload: true })
      const txid = await unstake(state.withdraw.lpAmount, state.withdraw.isExiting)
      if (!txid) throw new Error(`Transaction failed`)
      dispatch({ type: FuryFarmingWithdrawActionType.SET_TXID, payload: txid })
      onOngoingFarmingTxIdChange(txid, contractAddress)
      onNext(DefiStep.Status)
      dispatch({ type: FuryFarmingWithdrawActionType.SET_LOADING, payload: false })
