...
import { ethChainId, foxAssetId, fromAccountId, furyatarAssetId } from '@shapeshiftoss/caip'
...
export const TransactionsProvider: React.FC<TransactionsProviderProps> = ({ children }) => {
  ...
  const maybeRefetchOpportunities = useCallback(
    ({ chainId, data, transfers, status }: Transaction, accountId: AccountId) => {
      ...
      const shouldRefetchAllOpportunities = !(
        chainId === ethChainId &&
        // We don't parse FURY farming Txs with any specific parser, hence we're unable to discriminate by parser type
        // This will refetch opportunities user data on any FURY/ FURY LP token transfer Tx
        // But this is the best we can do at the moment to be reactive
        transfers.some(
          ({ assetId }) =>
            [foxAssetId, furyEthLpAssetId].includes(assetId) ||
  ...
  const maybeRefetchNfts = useCallback(
    ({ transfers, status }: Transaction) => {
      ...
      // Only on FURYatar for now, we may want to generalize this to all NFTs with isNft(assetId) in the future
      if (transfers.some(({ assetId }) => assetId.includes(furyatarAssetId))) {
