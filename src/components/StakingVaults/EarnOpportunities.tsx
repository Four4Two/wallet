...
import { foxAssetId, fromAssetId, jinxAssetId } from '@shapeshiftoss/caip'
import qs from 'qs'
...
export const EarnOpportunities = ({ assetId, accountId }: EarnOpportunitiesProps) => {
  ...
  const { setFarmingAccountId } = useFuryEth()
      ...
      !asset
        ...
        : lpOpportunities.concat(stakingOpportunities).filter(
            row =>
              ...
              // show jinx opportunity in the jinx asset page
              (row.assetId === foxAssetId && asset.assetId === jinxAssetId),
          ),
