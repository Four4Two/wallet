...
export const MainOpportunity = ({
  ...
  const hasActiveStaking = bnOrZero(jinxEarnOpportunityData?.stakedAmountCryptoBaseUnit).gt(0)

  const opportunityButtonTranslation = useMemo(() => {
    if (isDemoWallet || !wallet || !supportsETH(wallet)) return 'common.connectWallet'
    if (hasActiveStaking) return 'plugins.furyPage.manage'
    return 'plugins.furyPage.getStarted'
  }, [isDemoWallet, wallet, hasActiveStaking])

  const isOpportunityButtonReady = useMemo(
    () => Boolean(isDemoWallet || (wallet && !supportsETH(wallet)) || jinxEarnOpportunityData),
    [isDemoWallet, wallet, jinxEarnOpportunityData],
  )
      ...
      <Card.Header>
        <Flex flexDirection='row' alignItems='center' mb={2}>
          ...
          <Text
            ...
            color='inherit'
            translation={[
              'plugins.furyPage.mainStakingTitle',
              {
        ...
        <Text translation='plugins.furyPage.mainStakingDescription' color='gray.500' />
      </Card.Header>
      <Card.Body>
        <Flex
          width='full'
        ...
        >
          ...
          >
            <Text translation='plugins.furyPage.currentApy' color='gray.500' mb={1} />
          ...
          <Flex
            ...
            alignItems={{ base: 'center', md: 'flex-start' }}
          >
            <Text translation='plugins.furyPage.tvl' color='gray.500' mb={1} />
          ...
          <Flex
            ...
            alignItems={{ base: 'center', md: 'flex-start' }}
          >
            <Text translation='plugins.furyPage.balance' color='gray.500' mb={1} />
