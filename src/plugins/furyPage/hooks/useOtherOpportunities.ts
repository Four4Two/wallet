...
import { foxAssetId, fromAccountId, fromAssetId, jinxAssetId } from '@shapeshiftoss/caip'
...
export const useOtherOpportunities = (assetId: AssetId) => {
  const highestFarmingBalanceAccountIdFilter = useMemo(
    () => ({
      stakingId: furyEthStakingAssetIdV6 as StakingId,
    }),
    [],
  )
  ...
  const furyFarmingOpportunityMetadata = useMemo(
    () => stakingOpportunities[furyEthStakingAssetIdV6 as StakingId],
