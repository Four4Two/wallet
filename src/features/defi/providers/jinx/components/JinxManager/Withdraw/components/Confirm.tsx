...
export const Confirm: React.FC<StepComponentProps & { accountId?: AccountId | undefined }> = ({
  ...
  const withdrawalFee = useMemo(() => {
    return state?.withdraw.withdrawType === WithdrawType.INSTANT
      ? bnOrZero(
          bn(state?.withdraw.cryptoAmount ?? '0').times(state?.jinxFeePercentage ?? '0'),
        ).toString()
      : '0'
  }, [state?.withdraw.withdrawType, state?.withdraw.cryptoAmount, state?.jinxFeePercentage])
  ...
  const handleConfirm = useCallback(async () => {
    try {
      ...
      dispatch({ type: JinxWithdrawActionType.SET_TXID, payload: txid })
      onNext(DefiStep.Status)

      const transactionReceipt = await poll({
        fn: () => jinxApi.getTxReceipt({ txid }),
      ...
      dispatch({ type: JinxWithdrawActionType.SET_LOADING, payload: false })
    } catch (error) {
      console.error(error)
    }
  }, [
