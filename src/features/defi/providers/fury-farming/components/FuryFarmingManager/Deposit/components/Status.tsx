...
export const Status: React.FC<StatusProps> = ({ accountId }) => {
  ...
  if (!asset)
    throw new Error(`Asset not found for AssetId ${furyFarmingOpportunity?.underlyingAssetId}`)
  if (!feeAsset) throw new Error(`Fee asset not found for AssetId ${feeAssetId}`)
  ...
  useEffect(() => {
    if (!furyFarmingOpportunity) return
  ...
  if (!state || !dispatch || !furyFarmingOpportunity) return null
      ...
      case 'success':
        return {
          ...
          statusBody: translate('modals.deposit.status.success', {
            opportunity: furyFarmingOpportunity?.opportunityName,
          }),
          statusBg: 'green.500',
        }
      case 'failed':
  ...
  return (
    <TxStatus
      ...
      pairIcons={furyFarmingOpportunity?.icons}
    >
      <Summary>
