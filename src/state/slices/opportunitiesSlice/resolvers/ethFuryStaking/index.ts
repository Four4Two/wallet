...
export const ethFuryStakingMetadataResolver = async ({
  ...
  // apr
  const furyRewardRatePerTokenV6 = await rewardRatePerToken(furyFarmingContract)
  ...
  const furyEquivalentPerLPToken = token1PoolReservesEquivalent
    ...
    .times(bn(10).pow(pair.token1.decimals)) // convert to base unit value
    .toString()
  const apy = bnOrZero(makeTotalLpApr(furyRewardRatePerTokenV6, furyEquivalentPerLPToken))
    .div(100)
    .toString()
