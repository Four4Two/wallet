...
export const ClaimConfirm = ({
  ...
  // The highest level AssetId/OpportunityId, in this case of the single FURYy contract
  ...
  const undelegations = useMemo(
    () =>
      jinxEarnOpportunityData && supportsUndelegations(jinxEarnOpportunityData)
        ? jinxEarnOpportunityData.undelegations
        : undefined,
  ...
  const handleConfirm = useCallback(async () => {
    if (!(walletState.wallet && contractAddress && userAddress && jinxApi && bip44Params)) return
    setLoading(true)
    try {
      if (!supportsETH(walletState.wallet))
        throw new Error(`handleConfirm: wallet does not support ethereum`)
      const txid = await jinxApi.claimWithdraw({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!(walletState.wallet && contractAddress && jinxApi && chainAdapter)) return
        if (!supportsETH(walletState.wallet))
        ...
        const feeDataEstimate = await jinxApi.estimateClaimWithdrawFees({
          claimAddress: userAddress,
          userAddress,
          contractAddress,
          wallet: walletState.wallet,
